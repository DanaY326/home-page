...
export default function Model(props: Props) {
    ...
    return (
        <div className={"mb-6 md:mb-12 md:h-60 mx-2 text-neutral-500"}>
            ...
            <Image
                className="w-2/5 rounded-3xl my-10 max-width-full max-height-full"
                src={item.image}
                alt="Image"
                height={1000}
                width={1000}
            />
            <h2 className="h-40 md:h-25"><p className="text-neutral-300 text-3xl mb-1">{item.name}</p>{item.subtitle ? (<p className="text-lg">{item.subtitle}</p>) : ""}</h2>
            <p className="my-8 text-xs">TRY IT IN</p>
            {item.links ? (
                <ul className="mb-6 md:mb-8 h-30">
